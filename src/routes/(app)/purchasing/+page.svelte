<script lang="ts">
	...
	function listStatusBadge(p: Purchase) {
		...
		if (status === 'PARTIAL') {
			return `<span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300">${t('pr.status.partial', localeStore.value)}</span>`;
		}
		if (status === 'REJECTED') {
