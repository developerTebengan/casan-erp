	...
	async function handleSubmit(formData: Record<string, unknown>) {
		...
		try {
			...
			if (res.ok) {
				...
				toastStore.error(errorData.message || 'Failed to create product');
			}
		} finally {
			loading = false;
		}
...
<div class="space-y-6">
	<Breadcrumb items={[{ label: 'Inventory', href: '/inventory' }, { label: 'Add Product' }]} />

	<div>
		<h1 class="text-main text-2xl font-bold sm:text-3xl">Add Product</h1>
