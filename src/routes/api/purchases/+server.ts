...
export const GET: RequestHandler = async ({ url }) => {
	try {
		...
		const status =
			(url.searchParams.get('status') as 'DRAFT' | 'ORDERED' | 'RECEIVED' | 'CANCELLED') ||
			undefined;
		const page = Math.max(1, Number(url.searchParams.get('page') ?? 1));
		const limit = Math.min(100, Math.max(1, Number(url.searchParams.get('limit') ?? 10)));

		const service = purchaseService();
		const result = await service.list({ search, supplierId, status, page, limit });
		return json(result);
	} catch (e) {
		console.error(e);
...
export const POST: RequestHandler = async ({ request, locals }) => {
	try {
		...
		console.error(e);
		throw error(500, { message: 'Failed to create purchase order' });
	}
};
